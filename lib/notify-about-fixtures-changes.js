...
async function notifyAboutFixturesChanges(diffs) {
  console.log("");
  console.log(
    "🤖  Fixture changes detected in cron job. Creating pull request ..."
  );
  ...
  console.log(
    `🤖  Signed in as ${login}. Looking if I already created a pull request`
  );
  ...
  const { data: pullRequestsResult } = await github.get(
  ...
  if (pullRequestsResult.total_count > 1) {
    console.log(
      "🤖🆘 Oh oh, I don’t know how to handle more than one pull requests. Creating an issue for my human friends"
    );
    const result = await github.post(`/repos/${repoName}/issues`, {
      title: "🤖🆘 Too many PRs",
      body: `Dearest humans,

I’ve run into a problem here. My friend Travis notified me that something changed in GitHub’s APIs. I would usually create a new pull request to let you know about it, or update an existing one. But now there more than one: ${pullRequestNumbers
        .map(number => `#${number}`)
        .join(", ")}
...
Hope you can fix it (and my circuits) soon 🙏`
  ...
  if (pullRequestsResult.total_count === 1) {
    ...
    const { data } = await github.get(
      `/repos/${repoName}/pulls/${pullRequest.number}`
    );
  ...
  } = await github.get(`/repos/${repoName}/git/refs/heads/master`);
...
\`\`\`diff
${diffString(diff.oldNormalizedFixtures, diff.newNormalizedFixtures, {
  color: false
}).trim()}
\`\`\`
</details>`;
...
function updateFixtures({ diffs, github, repoName, branchName }) {
  return diffs.reduce(async (promise, diff) => {
    ...
    const normalizedFixtureContent = Buffer.from(
      JSON.stringify(diff.newNormalizedFixtures, null, 2) + "\n"
    ).toString("base64");
    const rawFixturePath = `/repos/${repoName}/contents/scenarios/${diff.name}/raw-fixture.json?ref=${branchName}`;
    const rawFixtureContent = Buffer.from(
      JSON.stringify(diff.newRawFixtures, null, 2) + "\n"
    ).toString("base64");

    if (diff.changes[0][0] === "-") {
      throw new Error(
        `🤖🆘 Looks like ${diff.name} is a new fixture, but that could not have come from a routine check?`
      );
    }

    console.log(
      `🤖  Loading current fixture file for ${diff.name} from ${normalizedFixturePath}`
    );
    try {
    ...
    const {
      data: {
        ...
        content: existingNormalizedFixtureContent
      }
    } = await github.get(normalizedFixturePath);
    const {
      data: { sha: rawFixtureSha }
    } = await github.get(rawFixturePath);

    const existingContent = Buffer.from(
      existingNormalizedFixtureContent,
      "base64"
    ).toString();
    const diffToRemote = getDiff(
      diff.newNormalizedFixtures,
    ...
    const {
      data: { content }
    } = await github.put(normalizedFixturePath, {
