...
function normalizeRepository (scenarioState, response, fixture) {
  // set all IDs to 1
  setIfExists(response, 'id', fixturizeEntityId.bind(null, scenarioState.ids, 'repository'))
  ...
  // normalize payload
  setIfExists(fixture, 'body.name', (name) => name.replace(temporaryRepository.regex, '$1'))
}
