...
function branchProtection(scenarioState, response) {
  ...
  const dismissalRestrictionsTeams =
      response?.required_pull_request_reviews?.dismissal_restrictions?.teams || [];
  const restrictionsUsers = response?.restrictions?.users || [];
  const restrictionsTeams = response?.restrictions?.teams || [];
