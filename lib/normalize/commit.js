...
function normalizeCommit (scenarioState, response) {
  ...
  setIfExists(response, 'url', url => url.replace(temporaryRepository.regex, '$1').replace(sha, fixturizedSha))
  setIfExists(response, 'html_url', url => url.replace(temporaryRepository.regex, '$1').replace(sha, fixturizedSha))
  setIfExists(response, 'tree.url', url => url.replace(temporaryRepository.regex, '$1').replace(treeSha, fixturizedTreeSha))
}
